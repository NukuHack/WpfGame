    ...
    public partial class MainWindow : Window
    {
        ...
        public void SetupTerrainScalingWithScrolling()
        {
            this.MouseWheel += (s, e) => {
                ...
                // Calculate the offset adjustments based on the mouse position
                double offsetXAdjustment = (mousePos.X / oldScale - mousePos.X / Scale);
                double offsetYAdjustment = mousePos.Y - ((mousePos.Y - offsetY) * (Scale / oldScale));
        ...
        public void SetupGetInfoFromTerrainOnLeftClick()
        {
            ...
            this.MouseLeftButtonDown += (s, e) => UpdateInfoDisplay(e);
        }
        ...
        public void MoveOffset(Direction direction)
        {
            switch (direction)
            {
                case Direction.Up: offsetY += 10; break;
                case Direction.Down: offsetY -= 10; break;
                case Direction.Right: offsetX += 10; break;
                case Direction.Left: offsetX -= 10; break;
            }
            RenderTerrain();
        }
        ...
        public void ShowDebugInfo()
        {
            if (doDebug)
            {
                if (noiseDebug[0, 0] == 0)
                {
                    ...
                    MessageBox.Show("you have to redraw/reload the map to log the values for it.\n(use wasd to redraw it)", "Terrain Debug Info");
                }
                else
                {
